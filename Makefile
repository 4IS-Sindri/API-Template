...
PKG_NAME := $(strip $(call GetFromPkg,name))
PKG_VERSION := $(strip $(call GetFromPkg,version))
PROJECT_URL := $(strip $(call GetFromPkg,homepage))
AUTHOR := $(strip $(call GetFromPkg,author.name))

# Set the APP_NAME variable to the value of the name property without the scope.
APP_NAME := $(strip $(shell echo $(PKG_NAME) | cut -d'/' -f 2))
